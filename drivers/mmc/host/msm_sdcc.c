...
#define IRQ_DEBUG 0
#define SDC3_INDEX 1
#define MSM_MMC_SDC3_IDLE_TIMEOUT	30000 /* msecs */
...
int sysctl_sdcard_log = 0;  /* Debug Console Flag OFF:0 ON:Other */
...
msmsdcc_start_command(struct msmsdcc_host *host, struct mmc_command *cmd, u32 c)
{
	...
	if (host->mmc->index == SDC_SDCARD_INDEX && cmd && SDC_SDCARD_LOG(SDC_SDCARD_LOG_HOST) ) {
		printk(KERN_INFO "%s : SD-HOST INFO : cmd=%u, arg=%08X\n",
			mmc_hostname(host->mmc), (u32)c & SDC_SDCARD_CMD_MASK, (u32)cmd->arg);
	}
...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if ( SDC_SDCARD_LOG(SDC_SDCARD_LOG_IRQ) ) {
			if (mmc->index == SDC3_INDEX && host->curr.cmd) {
				cmd = host->curr.cmd;
				printk(KERN_INFO "%s : SD-IRQ : status=%08X, cmd=%d, arg=%08X\n",
					mmc_hostname(mmc), (u32)status, (u32)cmd->opcode, (u32)cmd->arg);
			}
		}
...
static int msmsdcc_get_ro(struct mmc_host *mmc)
{
	...
	if( mmc->card->csd.perm_write_protect || mmc->card->csd.tmp_write_protect )
	{
		status = 1;
	}
	return status;
...
#endif
	if(mmc->index == SDC3_INDEX) {
		host->idle_tout_ms = MSM_MMC_SDC3_IDLE_TIMEOUT;
	} else {
