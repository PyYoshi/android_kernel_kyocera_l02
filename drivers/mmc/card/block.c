	...
	struct mmc_ioc_cmd __user *ic_ptr)
{
	...
	if (card->host->index == SDC_SDCARD_INDEX && SDC_SDCARD_LOG(SDC_SDCARD_LOG_IOCTL)) {
		printk(KERN_INFO "%s : SD-IOCTL START : idata=%08X, cmd=%u, arg=%08X, cflags=%08X, dflags=%08X\n",
			mmc_hostname(card->host), (u32)idata, (u32)idata->ic.opcode, (u32)idata->ic.arg,
		    (u32)idata->ic.flags, (u32)data.flags);
	}
	
    mmc_wait_for_req(card->host, &mrq);
	
	if (card->host->index == SDC_SDCARD_INDEX && SDC_SDCARD_LOG(SDC_SDCARD_LOG_IOCTL)) {
		printk(KERN_INFO "%s : SD-IOCTL END   : idata=%08X, cmd_result=%d, data_result=%d\n",
			mmc_hostname(card->host), (u32)idata, cmd.error, data.error);
	}
...
cmd_done:
	if (card) {
		if (card->host->index == SDC_SDCARD_INDEX && err) {
			printk(KERN_ERR "%s : SD-IOCTL ERROR : idata=%08X, cmd=%u, arg=%08X, cflags=%08X, dflags=%08X, err=%d\n",
				mmc_hostname(card->host), (u32)idata, (u32)idata->ic.opcode, (u32)idata->ic.arg,
				(u32)idata->ic.flags, (u32)data.flags, err);
		}
	}
	mmc_blk_put(md);
	kfree(idata->buf);
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	do {
		...
		if (card->host->index == SDC_SDCARD_INDEX && status != MMC_BLK_SUCCESS) {
			printk(KERN_ERR   "%s : SD-CARD ERROR %s :req=%08X, secpos=%u, secno=%u, result=%d\n",
				mmc_hostname(card->host), SDC_GET_DIRSTR(req), (u32)req, (u32)blk_rq_pos(req), (u32)blk_rq_sectors(req), status );
		}
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (req && req->cmd_flags & REQ_SANITIZE) {
	    ...
	    if (card->host->index == SDC_SDCARD_INDEX && req && SDC_SDCARD_LOG(SDC_SDCARD_LOG_CARD) ) {
		    printk(KERN_INFO "%s : SD-CARD START %s :req=%08X, secpos=%u, secno=%u\n",
				mmc_hostname(card->host), SDC_GET_DIRSTR(req), (u32)req, (u32)blk_rq_pos(req), (u32)blk_rq_sectors(req) );
		}
		
		ret = mmc_blk_issue_rw_rq(mq, req);
		
		if (card->host->index == SDC_SDCARD_INDEX && req && SDC_SDCARD_LOG(SDC_SDCARD_LOG_CARD) ) {
			printk(KERN_INFO "%s : SD-CARD END   %s :result=%d\n",
				mmc_hostname(card->host), SDC_GET_DIRSTR(req), ret);
		}
	}
