...
static int __mmc_start_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (host->index == SDC_SDCARD_INDEX && mrq->cmd && SDC_SDCARD_LOG(SDC_SDCARD_LOG_CORE) ) {
	    cmd = mrq->cmd;
		printk(KERN_INFO "%s : SD-CORE INFO : cmd=%u, arg=%08X\n", mmc_hostname(host), (u32)cmd->opcode, (u32)cmd->arg);
	}
				  ...
				  struct mmc_request *mrq)
{
	...
	while (1) {
		...
		if (host->index == SDC_SDCARD_INDEX) {
			pr_err("%s : SD-CORE ERROR : cmd=%u, arg=%08X, err=%d, retrying...\n",
				mmc_hostname(host), (u32)cmd->opcode, (u32)cmd->arg, cmd->error);
		}
		cmd->retries--;
		cmd->error = 0;
		host->ops->request(host, mrq);
