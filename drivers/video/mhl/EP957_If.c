...
// ADAPTOE_ID_H, L should be modified by customers
	...
	0x01, // 6: AUD_LINK_MODE	- AUD_8CH[1], AUD_2CH[0]
...
EP_LOG_FUNCTION_NAME_ENTRY	
	...
	Temp_Data[0] = 37; // default 47, set to 42 for E version
#endif
...
BOOL MHL_MSC_Cmd_READ_DEVICE_CAP(BYTE offset, PBYTE pValue)
{
	...
	for(i=0; i<MHL_CMD_RETRY_TIME; ++i) {
		...
		error = MHL_CBus_RQ_Check(1, pValue);  //modify 130902 (for MHL CTS 1.3)
...
RQ_STATUS MHL_CBus_RQ_Go_DDC(void) // for pass test read EDID
{
	...
	Temp_Data[0] = 0x01; 
	if(Device_Capability_Default[0x0A] | 0x04) {		//modify 131205
		Temp_Data[0] |= EP957_CBUS_RQ_Control__WB_SPT;	//modify 131205
	}													//modify 131205
...
#endif
		mdelay(30); // delay 30ms  //modify 130902 (for MHL CTS 1.3)
	}
	...
	// Or polling every < 200ms is also available.
	{
		...
		if(EP957Control_RCP_RAP_Read(RCP_RAP_Code) == TRUE) 
		{
			if(RCP_RAP_Code[0] == MSC_RAP)
			{
				...
				// modify 131127 (for CTS item 3.2.8.1)
				//--- start ---
...
{
       /*EP957B Monitoling*/
	EP957_Reg_Read(EP957_TX_PHY_Control_0, &Temp_Data[0], 1);
//	EP_DEV_DBG("EP957_TX_PHY_Control_0 = 0x%0X\n",Temp_Data[0]);
	if((Temp_Data[0] & 0x01) == 0) {
