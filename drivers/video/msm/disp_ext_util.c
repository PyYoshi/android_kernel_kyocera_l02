...
void disp_ext_util_set_kcjprop( struct fb_kcjprop_data* kcjprop_data )
{
...
#ifdef CONFIG_FB_MSM_MIPI_RENESAS_VIDEO_720P_PT
        if(kcjprop_data->rw_display_cabc > 1) {
#else
    ...
    if( kcjprop_data->rw_display_gamma_normal_valid == 0 ){
        mipi_renesas_video_gamma_Ap_set( kcjprop_data->rw_display_gamma_normal_r_plus  );
        mipi_renesas_video_gamma_Am_set( kcjprop_data->rw_display_gamma_normal_r_minus );
        mipi_renesas_video_gamma_Bp_set( kcjprop_data->rw_display_gamma_normal_g_plus  );
        mipi_renesas_video_gamma_Bm_set( kcjprop_data->rw_display_gamma_normal_g_minus );
        mipi_renesas_video_gamma_Cp_set( kcjprop_data->rw_display_gamma_normal_b_plus  );
        mipi_renesas_video_gamma_Cm_set( kcjprop_data->rw_display_gamma_normal_b_minus );
        DISP_LOCAL_LOG_EMERG("%s:mipi_renesas_video_gamma set\n", __func__);
    }
#endif /*CONFIG_FB_MSM_MIPI_RENESAS_VIDEO_720P_PT*/
