...
static char detect_board_protect_off[4] = { 0xB0, 0x00, DTYPE_GEN_WRITE2, 0x80 };
static char detect_board_protect_on[4]  = { 0xB0, 0x03, DTYPE_GEN_WRITE2, 0x80 };
static char maximum_return_seze_set[4] = { 0x06, 0x00, DTYPE_MAX_PKTSIZE  , 0x80 };
static char detect_board_mipi_lane_read[4] = { 0xBF, 0x00, DTYPE_GEN_READ1  , 0xA0 };
...
int disp_ext_board_detect_board(struct msm_fb_data_type *mfd)
{
...
#ifdef CONFIG_FB_MSM_MIPI_RENESAS_VIDEO_720P_PT
	/* protect off */
	ret = disp_ext_board_cmd_tx( detect_board_protect_off, sizeof(detect_board_protect_off), 0 );
#else
	...
	for( i = 0 ; i < DETECT_BOARD_NUM ; i++ ) {
...
#ifdef CONFIG_FB_MSM_MIPI_RENESAS_VIDEO_720P_PT
            disp_ext_board_cmd_tx( detect_board_protect_on, sizeof(detect_board_protect_on), 0 );
#else
			mipi_dsi_op_mode_config(mfd->panel_info.mipi.mode);
#endif  /* CONFIG_FB_MSM_MIPI_RENESAS_VIDEO_720P_PT */
			light_led_disp_set(LIGHT_MAIN_WLED_LCD_DIS);
        ...
        if ( (data[0]==0x01223306) && (data[1]==0xB0000000) ) {
            panel_found = 1;
            break;
        }
#else
