...
{
	int32_t rc = -EFAULT;
	uint8_t buf[client->addr_type + num_byte];
	uint16_t len = 0;
	uint16_t i;
	...
	S_I2C_DBG("%s reg addr = 0x%x data type: %d\n",
		__func__, addr, MSM_CAMERA_I2C_BYTE_DATA);
	if (client->cci_client) {
		struct msm_camera_cci_ctrl cci_ctrl;
		struct msm_camera_i2c_reg_conf reg_conf_tbl;
		reg_conf_tbl.reg_addr = addr;
		reg_conf_tbl.reg_data = data[0];
		cci_ctrl.cmd = MSM_CCI_I2C_WRITE;
		cci_ctrl.cci_info = client->cci_client;
		cci_ctrl.cfg.cci_i2c_write_cfg.reg_conf_tbl = &reg_conf_tbl;
...
int32_t msm_camera_i2c_burst_write_tbl(struct msm_camera_i2c_client *client,
	struct msm_camera_i2c_reg_conf *reg_conf_tbl, uint16_t size)
{
	int i;
	int32_t rc = -EFAULT;
	if (client->cci_client) {
		...
		if (reg_conf_tbl->cmd_type == MSM_CAMERA_I2C_CMD_POLL) {
			...
			uint16_t data[size];
			if (reg_conf_tbl->dt == 0)
				dt = MSM_CAMERA_I2C_BURST_DATA;
			else
				dt = reg_conf_tbl->dt;
			switch (dt) {
			case MSM_CAMERA_I2C_BURST_DATA:
				for (i = 0; i < size; i++)
					data[i] = reg_conf_tbl[i].reg_data;
				rc = msm_camera_i2c_burst_write(client,
					reg_conf_tbl->reg_addr, data, size);
				break;
			default:
				pr_err("%s: Unsupport data type: %d\n", __func__, dt);
				break;
...
//		array[index].size, array[index].data_type);
	if (array[index].data_type == MSM_CAMERA_I2C_BURST_DATA){
		rc = msm_camera_i2c_burst_write_tbl(client,
			(struct msm_camera_i2c_reg_conf *) array[index].conf,
			array[index].size);
	} else {
		rc = msm_camera_i2c_write_tbl(client,
