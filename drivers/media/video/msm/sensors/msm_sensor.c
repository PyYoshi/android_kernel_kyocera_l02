...
void msm_sensor_delay_frames(struct msm_sensor_ctrl_t *s_ctrl)
{
	long fps = 0;
	uint32_t delay = 0;
...
//						s_ctrl);
				rc =
					s_ctrl->func_tbl->
					sensor_set_auto_focus(s_ctrl, cdata.cfg.output_mode);
			}
			break;
...
// default = 1000ms
		...
		case CFG_SET_WB:
			if (s_ctrl->func_tbl->sensor_set_white_balance != NULL) {
				rc = s_ctrl->func_tbl->sensor_set_white_balance(s_ctrl, cdata.cfg.wb_val);
			}
			break;

		case CFG_SET_EXPOSURE_COMPENSATION:
			if (s_ctrl->func_tbl->sensor_set_exposure_compensation != NULL) {
				rc = s_ctrl->func_tbl->sensor_set_exposure_compensation(s_ctrl, cdata.cfg.exp_compensation);
			}
			break;

		case CFG_SET_BESTSHOT_MODE:
			if (s_ctrl->func_tbl->sensor_set_bestshot != NULL) {
				rc = s_ctrl->func_tbl->sensor_set_bestshot(s_ctrl, cdata.cfg.scene_val);
			}
			break;

		case CFG_SET_LOW_LIGHT:
			if (s_ctrl->func_tbl->
			sensor_set_low_light != NULL) {
				rc =
					s_ctrl->func_tbl->
					sensor_set_low_light (
						s_ctrl);
			}
			break;

		case CFG_CANCEL_AUTO_FOCUS:
			if (s_ctrl->func_tbl->sensor_cancel_auto_focus != NULL) {
				s_ctrl->func_tbl->sensor_cancel_auto_focus(s_ctrl);
			}
			break;

		case CFG_SET_FOCUS_MODE:
			if (s_ctrl->func_tbl->sensor_set_focus_mode != NULL) {
				rc = s_ctrl->func_tbl->sensor_set_focus_mode(
						s_ctrl, cdata.cfg.focus_mode);
			}
			break;

		case CFG_SET_CAF:
			if (s_ctrl->func_tbl->sensor_set_caf != NULL) {
//				rc = s_ctrl->func_tbl->sensor_set_caf(
//						s_ctrl, cdata.cfg.fd_enable);
				rc = s_ctrl->func_tbl->sensor_set_caf(
						s_ctrl, cdata.cfg.caf_info);
			}
			break;

		case CFG_SET_TOUCH_FOCUS:
			if (s_ctrl->func_tbl->sensor_set_touch_focus != NULL) {
				s_ctrl->func_tbl->sensor_set_touch_focus(
						s_ctrl, cdata.cfg.touch_info);
			}
			break;

		case CFG_SET_AEC_LOCK:
			if (s_ctrl->func_tbl->sensor_set_aec_lock != NULL) {
				rc = s_ctrl->func_tbl->sensor_set_aec_lock(
						s_ctrl, cdata.cfg.aec_lock);
			}
			break;

		case CFG_SET_AWB_LOCK:
			if (s_ctrl->func_tbl->sensor_set_awb_lock != NULL) {
				rc = s_ctrl->func_tbl->sensor_set_awb_lock(
						s_ctrl, cdata.cfg.awb_lock);
			}
			break;

		case CFG_GET_LENS_POSITION:
			if (s_ctrl->func_tbl->sensor_get_lens_position != NULL) {
				rc = s_ctrl->func_tbl->sensor_get_lens_position(
						s_ctrl, &cdata.cfg.lens_position);
				if(rc >= 0){
		...
		case CFG_CAPTURE_PRESET:
			if (s_ctrl->func_tbl->sensor_capture_preset != NULL) {
				rc = s_ctrl->func_tbl->sensor_capture_preset(s_ctrl,
					cdata.cfg.capture_size.width,
					cdata.cfg.capture_size.height);
			}
			break;

		case CFG_CAPTURE_POSTSET:
			if (s_ctrl->func_tbl->sensor_capture_postset != NULL) {
				rc = s_ctrl->func_tbl->sensor_capture_postset(s_ctrl);
			}
			break;
		...
		case CFG_AUTO_BRACKET:
			if (s_ctrl->func_tbl->sensor_auto_bracket != NULL) {
				rc = s_ctrl->func_tbl->sensor_auto_bracket(s_ctrl,
					cdata.cfg.auto_bracket_value);
			}
			break;
		case CFG_SET_FLICKER:
			if (s_ctrl->func_tbl->
			sensor_set_flicker_detect != NULL) {
				rc =
					s_ctrl->func_tbl->
					sensor_set_flicker_detect(
						s_ctrl,cdata.cfg.flicker);
			}
			break;
		case CFG_GET_ISP_SETTING:
			if (s_ctrl->func_tbl->
			sensor_get_isp_settings!= NULL) {
				rc =
					s_ctrl->func_tbl->sensor_get_isp_settings(s_ctrl,
					&cdata.cfg.isp_settings);
				if(rc >= 0){
					if (copy_to_user((void *)argp, &cdata, sizeof(struct sensor_cfg_data))) {
						rc = -EFAULT;
					}
            ...
            break;
		case CFG_GET_BRACKET_EXP_TIME:
// default = 1000ms
			cdata.cfg.bracket_exp_time[0] = 1000;
			cdata.cfg.bracket_exp_time[1] = 1000;
			cdata.cfg.bracket_exp_time[2] = 1000;
			if (s_ctrl->func_tbl->sensor_get_bracket_exposure_time != NULL) {
				rc = s_ctrl->func_tbl->sensor_get_bracket_exposure_time(
						s_ctrl,
						&cdata.cfg.bracket_exp_time[0]);
			}
			...
			if (copy_to_user((void *)argp,
				...
				rc = -EFAULT;
			break;
		case CFG_SET_FLICKER_LIB:
			if (s_ctrl->func_tbl->
			sensor_set_flicker_lib!= NULL) {
				rc =
					s_ctrl->func_tbl->sensor_set_flicker_lib(s_ctrl,
					&cdata.cfg.set_flicker_lib);
			}
			break;
		case CFG_GET_FLICKER_LIB:
			if (s_ctrl->func_tbl->
			sensor_get_flicker_lib!= NULL) {
				rc =
					s_ctrl->func_tbl->sensor_get_flicker_lib(s_ctrl,
					&cdata.cfg.get_flicker_lib);
				if(rc >= 0){
					if (copy_to_user((void *)argp, &cdata, sizeof(struct sensor_cfg_data))) {
						rc = -EFAULT;
					}
	...
	if(cdata.cfgtype == CFG_POWER_DOWN) {
		if (s_ctrl->func_tbl->sensor_scheduler_destroy != NULL) {
			s_ctrl->func_tbl->sensor_scheduler_destroy(s_ctrl);
		}
	}
	...
	const struct i2c_device_id *id)
{
	...
	if (s_ctrl->func_tbl->sensor_init != NULL) {
		s_ctrl->func_tbl->sensor_init(s_ctrl);
	}
...
void msm_sensor_get_lx_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	if (s_ctrl->func_tbl->sensor_get_lx_data != NULL) {
		s_ctrl->func_tbl->sensor_get_lx_data(s_ctrl);
	}
}
