...
struct pm8921_chg_chip {
	...
	struct work_struct		handle_wlc_charge_work;
};
...
static uint32_t oem_wlc_charge_flag = 0;
static uint32_t oem_wlc_late_charge_flag = 0;

static enum power_supply_property msm_wlc_power_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
};

#define OEM_WLC_BATT_TEMP_LOW_THRESHOLD   1000
#define OEM_WLC_BATT_TEMP_HIGH_THRESHOLD  1250
#define OEM_WIRELESS_WAKELOCK_MSEC        2000
#define OEM_WLC_BATT_TEMP_SHIFT           8
#define OEM_WLC_ROUND_THRESHOLD           (1 << (OEM_WLC_BATT_TEMP_SHIFT - 1))
#define OEM_WLC_BATT_TEMP_OUTPUT          (300 << OEM_WLC_BATT_TEMP_SHIFT)
#define OEM_WLC_BATT_TEMP_MAX             (450 << OEM_WLC_BATT_TEMP_SHIFT)
#define OEM_WLC_BATT_TEMP_FOCTOR          300
#define OEM_WLC_BATT_TEMP_CHATTERING_NUM  5

struct wake_lock oem_wireless_charge_wake_lock;
struct wake_lock oem_wireless_detect_wake_lock;
struct workqueue_struct *oem_wlc_chk_queue;
static int oem_wlc_prev_batt_temp = OEM_WLC_BATT_TEMP_OUTPUT;

static bool oem_wlc_is_charging(struct pm8921_chg_chip *chip);
static void oem_wlc_charge_state_check(struct pm8921_chg_chip *chip);
static int oem_wlc_batt_temp_adjust(void);
static int oem_wlc_mode_check(int batt_temp);
...
static int is_battery_charging(int fsm_state)
{
	...
	if (pm8921_is_wireless_charging())
		return 1;
...
static int get_prop_batt_temp(struct pm8921_chg_chip *chip, int *temp)
{
	...
	if (result.physical > MAX_TOLERABLE_BATT_TEMP_DDC) {
		if (!oem_wlc_mode_check(result.physical)){
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_HEALTH:
		if (pm8921_is_wireless_charging() || oem_wlc_late_charge_flag) {
...
static irqreturn_t bat_temp_ok_irq_handler(int irq, void *data)
{
	...
	wake_lock_timeout(&oem_wireless_detect_wake_lock, 
	                  msecs_to_jiffies(OEM_WIRELESS_WAKELOCK_MSEC));
	queue_work(oem_wlc_chk_queue, &chip->handle_wlc_charge_work);
...
static int oem_hkadc_init_battery_temp(void)
{
	...
	do {
		...
		if (rc) {
			...
			work = (int)result.physical;
			if(work >= MAX_TOLERABLE_BATT_TEMP_DDC)
				chattering++;
		}

		if(oem_wlc_mode_check((int)result.physical)) {
			work = oem_wlc_batt_temp_adjust();
			chattering = 0;
		}
	} while(chattering && chattering < OEM_WLC_BATT_TEMP_CHATTERING_NUM);
	...
	for (cnt=0;cnt<HKADC_BAT_TEMP_ELEMENT_COUNT;cnt++) {
		do {
			...
			if (rc) {
				...
				if(oem_wlc_mode_check((int)result.physical)) {
					battemp_new = oem_wlc_batt_temp_adjust();
					bat_temp_work_tmp[cnt] = battemp_new;
					chattering = 0;
				} else {
					battemp_new = (int)result.physical;
					bat_temp_work_tmp[cnt] = (int)result.physical;
					if(battemp_new >= MAX_TOLERABLE_BATT_TEMP_DDC)
						chattering++;
				}
			}
		} while(chattering && chattering < OEM_WLC_BATT_TEMP_CHATTERING_NUM);
...
static void oem_judge_charge_state(void)
{
	do {
		if (pm8921_is_wireless_charging()){
			charge_state = CHG_STATE_WIRELESS;
			break;
		}
...
static void oem_hkadc_monitor(struct work_struct *work)
{
	...
	oem_wlc_charge_state_check(the_chip);
...
EXPORT_SYMBOL(kcbms_get_substrate_therm);

int32_t kcbms_get_battery_therm(int32_t *temp){

	int ret;

	ret = oem_hkadc_master_data_read(POWER_SUPPLY_PROP_TEMP, temp);
	pr_debug("ret = %d battery_therm = %d\n", ret, *temp);

	return ret;
}
EXPORT_SYMBOL(kcbms_get_battery_therm);
...
static bool oem_wlc_is_charging(struct pm8921_chg_chip *chip)
{
	...
	if (chip->wlc_psy.get_property(&chip->wlc_psy,
					POWER_SUPPLY_PROP_ONLINE, &ret)) {
		pr_debug("wlc get_property ERROR!!\n");
		return false;
	}
	
	pr_debug("wlc status %d\n", ret.intval);
	return ret.intval;
}

int pm8921_is_wireless_charging(void)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return oem_wlc_is_charging(the_chip);
}
EXPORT_SYMBOL(pm8921_is_wireless_charging);

static int oem_wlc_power_get_property(struct power_supply *psy,
				       enum power_supply_property psp,
				       union power_supply_propval *val)
{
	int result = 0;
	
	pr_debug("wlc get_property : psp %d\n", psp);
	
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = oem_wlc_charge_flag;
		pr_debug("WLC ONLINE:%d\n", val->intval);
		break;
...
static int oem_wlc_power_set_property(struct power_supply *psy,
				       enum power_supply_property psp,
				       const union power_supply_propval *val)
{
	int result = 0;
	
	pr_debug("wlc set_property : psp %d\n", psp);
	
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		if (val->intval == 0 || val->intval == 1){
			oem_wlc_charge_flag = val->intval;
		}
		else{
			oem_wlc_charge_flag = 0;
			result = -EINVAL;
		}
		pr_debug("WLC ONLINE:%d\n", oem_wlc_charge_flag);
		
		power_supply_changed(&the_chip->batt_psy);
		power_supply_changed(&the_chip->wlc_psy);
		break;
...
static int oem_wlc_mode_check(int batt_temp)
{
	...
	if(batt_temp >= OEM_WLC_BATT_TEMP_LOW_THRESHOLD && batt_temp < OEM_WLC_BATT_TEMP_HIGH_THRESHOLD) {
		mode = 1;
	}
	else {
		mode = 0;
...
static void oem_wlc_set_flag(void)
{
	uint8_t pbuf = 1;
	...
	kdnand_id_write(DNAND_ID_KERNEL_18, 150, &pbuf, sizeof(uint8_t));
	oem_param_bms.wlc_flag=1;
}
...
static void oem_wlc_charge_state_check(struct pm8921_chg_chip *chip)
{
	int batt_temp = 0;
	int wlc_mode = 0;
	static uint32_t wlc_off_check_cnt = 0;

	batt_temp = pm8921_batt_temperature();
	pr_debug("batt_temp %d\n", batt_temp);

	wlc_mode = oem_wlc_mode_check(batt_temp);
	
	if(wlc_mode) {
		power_supply_set_online(&chip->wlc_psy, true);
		oem_wlc_late_charge_flag = 1;
		wlc_off_check_cnt = 0;
		oem_wlc_set_flag();
		wake_lock(&oem_wireless_charge_wake_lock);
	}
	
	else {
		if (oem_pm8921_bms_get_wireless_eoc()) {
			chip->bms_notify.is_battery_full = 1;
		}
		else {
			chip->bms_notify.is_battery_full = 0;
		...
		if(wlc_off_check_cnt < 0xFFFFFFFF) wlc_off_check_cnt++;
		if(wlc_off_check_cnt > 3) oem_wlc_late_charge_flag = 0;
		kcbms_get_battery_therm(&batt_temp);
		oem_wlc_prev_batt_temp = batt_temp << OEM_WLC_BATT_TEMP_SHIFT;
		wake_unlock(&oem_wireless_charge_wake_lock);
	}
	bms_notify_check(chip);
}

static void oem_wlc_check_worker(struct work_struct *work)
{
	int batt_temp = 0;
	int wlc_mode = 0;
...
static int oem_wlc_batt_temp_adjust(void)
{
	int result_batt_temp;
	int32_t substrate_therm = OEM_WLC_BATT_TEMP_OUTPUT;

	kcbms_get_substrate_therm(&substrate_therm);

	pr_debug("batt_temp = %d substrate_therm = %d \n", (oem_wlc_prev_batt_temp >> OEM_WLC_BATT_TEMP_SHIFT), substrate_therm);

	substrate_therm = (substrate_therm * 10) << OEM_WLC_BATT_TEMP_SHIFT;

	pr_debug("oem_wlc_prev_batt_temp = %d substrate_therm = %d \n", oem_wlc_prev_batt_temp, substrate_therm);

	oem_wlc_prev_batt_temp = ((oem_wlc_prev_batt_temp) * (OEM_WLC_BATT_TEMP_FOCTOR - 1 ) + substrate_therm) / OEM_WLC_BATT_TEMP_FOCTOR;
	
	pr_debug("oem_wlc_prev_batt_temp = %d \n", oem_wlc_prev_batt_temp);
	if(oem_wlc_prev_batt_temp > OEM_WLC_BATT_TEMP_MAX)
		oem_wlc_prev_batt_temp = OEM_WLC_BATT_TEMP_MAX;

	result_batt_temp = (oem_wlc_prev_batt_temp + OEM_WLC_ROUND_THRESHOLD) >> OEM_WLC_BATT_TEMP_SHIFT;
	pr_debug("wlc batt_temp = %d \n", result_batt_temp);

	return result_batt_temp;
}
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	chip->wlc_psy.name = "wlc";
	chip->wlc_psy.type = POWER_SUPPLY_TYPE_WLC;
	chip->wlc_psy.properties = msm_wlc_power_props;
	chip->wlc_psy.num_properties = ARRAY_SIZE(msm_wlc_power_props);
	chip->wlc_psy.get_property = oem_wlc_power_get_property;
	chip->wlc_psy.set_property = oem_wlc_power_set_property;
	...
	rc = power_supply_register(chip->dev, &chip->wlc_psy);
	if (rc < 0) {
		pr_err("power_supply_register wlc failed rc = %d\n", rc);
		goto unregister_bms;
	}
	...
	INIT_WORK(&chip->handle_wlc_charge_work, oem_wlc_check_worker);
	oem_wlc_chk_queue = create_workqueue("oem_wlc_chk_wq");
	...
	wake_lock_init(&oem_wireless_charge_wake_lock, WAKE_LOCK_SUSPEND, "oem_wireless_charge");
	wake_lock_init(&oem_wireless_detect_wake_lock, WAKE_LOCK_SUSPEND, "oem_wireless_detect");

	oem_wlc_charge_state_check(chip);
