...
/* Wireless charge parm */
#define OEM_WLC_ICHARGE_950           950000
#define OEM_WLC_ICHARGE_850           850000
#define OEM_WLC_ICHARGE_750           750000
#define OEM_WLC_ICHARGE_650           650000
#define OEM_WLC_ICHARGE_500           500000
#define OEM_WLC_ICHARGE_450           450000
#define OEM_WLC_ICHARGE_400           400000
#define OEM_WLC_ICHARGE_375           375000
#define OEM_WLC_ICHARGE_350           350000
#define OEM_WLC_ICHARGE_300           300000
#define OEM_WLC_ICHARGE_250           250000
#define OEM_WLC_ICHARGE_200           200000
#define OEM_WLC_ICHARGE_150           150000
#define OEM_WLC_ICHARGE_100           100000
#define OEM_WLC_ICHARGE_050           50000
#define OEM_WLC_ICHARGE_DEFAULT       OEM_WLC_ICHARGE_750
#define OEM_WLC_ICHARGE_HIGH          OEM_WLC_ICHARGE_850
#define OEM_WLC_ICHARGE_VERY_HIGH     OEM_WLC_ICHARGE_950
#define OEM_WLC_ICHARGE_LOW           OEM_WLC_ICHARGE_375
#define OEM_WLC_ICHARGE_VERY_LOW      OEM_WLC_ICHARGE_250
#define OEM_WLC_OCV_AVG_CNT           10
#define OEM_WLC_OCV_CNT_MAX           100
#define OEM_WLC_OCV_MA_CNT            15 /* 20s*15 = 300s */
#define OEM_WLC_EOC_CNT               1
#define OEM_WLC_EOC_SOC               99
#define OEM_WLC_EOC_VBATT_UV          4300000
#define OEM_WLC_CV_SOC                95
#define OEM_WLC_BMS_START_TIME        120
#define OEM_WLC_SOC_LOW_DELTA         5
#define OEM_WLC_SOC_MID_DELTA         10
#define OEM_WLC_SOC_HIGH_DELTA        15
#define OEM_WLC_SOC_ACCEPTABLE        3
#define OEM_WLC_SOC_THRESHOLD         45
#define OEM_WLC_ICHG_DECREASE_RATE    975
#define OEM_WLC_ICHG_DECREASE_SCALE   1000
#define OEM_WLC_SHUTDOWN_TIME         600

static int oem_wlc_ocv_est_work[OEM_WLC_OCV_MA_CNT] = {0};
static int oem_wlc_ocv_ma_index = 0;
static int oem_wlc_icharge     = 0;
static int oem_wlc_eoc_flag    = 0;
static int oem_wlc_cc_uah_total = 0;
static int oem_wlc_icharge_at_cv = OEM_WLC_ICHARGE_DEFAULT;
static int oem_wlc_set_shutdown_time_flag = 0;
static unsigned long oem_wlc_shutdown_time = 0;
static unsigned long oem_wlc_boot_time = 0;
module_param_cb(oem_wlc_shutdown_time, &bms_ro_param_ops, &oem_wlc_shutdown_time, 0644);
module_param_cb(oem_wlc_boot_time, &bms_ro_param_ops, &oem_wlc_boot_time, 0644);

static int oem_wlc_bms_is_wireless_charging(void);
static int oem_wlc_bms_estimate_wlc_ocv(int *wlc_ocv, int wlc_ichg);
static void oem_wlc_bms_calculate_wlc_ocv(int in_soc, int batt_temp, int chargecycles);
static void oem_wlc_bms_icharge_adjust(int in_soc, int soc_est);
static void oem_wlc_bms_recalculate_cc_uah(int *cc_uah, int delta_time_s);
static void oem_wlc_bms_eoc_check(int soc);
static void oem_wlc_charging_adjustments(struct pm8921_bms_chip *chip, int soc, int ibat_ua, int cc_uah);
static void oem_wlc_adjust_ocv_and_cc_in_cv(struct pm8921_bms_chip *chip,
				int soc, int batt_temp, int chargecycles,
				int fcc_uah, int cc_uah, int uuc_uah);
static int oem_wlc_scale_soc_while_chg(int new_soc, int prev_soc);
static int oem_wlc_bms_correct_soc(int chg_state, int in_soc);

static uint32_t oem_wlc_debug_level = 1;
#define oem_wlc_print(level, x...)			\
	do {						\
		if (oem_wlc_debug_level >= (level))	\
			pr_err(x);			\
		else					\
			pr_debug(x);			\
	} while (0)
...
static int get_batt_temp(struct pm8921_bms_chip *chip, int *batt_temp)
{
	...
	if(oem_wlc_bms_is_wireless_charging()) {
		kcbms_get_battery_therm(batt_temp);
		oem_wlc_print(7, "wireless batt temp = %d \n", *batt_temp);
		return 0;
	}
						...
						int *iavg_ua)
{
	...
	*remaining_charge_uah = calculate_remaining_charge_uah(chip, raw,
	...
	/* recalculate cc for wlc */
	oem_wlc_bms_recalculate_cc_uah(cc_uah, delta_time_s);
	oem_wlc_print(3, "recalculate cc_uah = %duAh\n", *cc_uah);
				...
				int fcc_uah, int cc_uah, int uuc_uah)
{
	...
	if (chip->soc_at_cv == -EINVAL) {
		...
		if (vbat_batt_terminal_uv >= chip->max_voltage_uv) {
#else
		if (vbat_batt_terminal_uv >= max_vol_uv && !oem_wlc_bms_is_wireless_charging()) {
#endif
			chip->soc_at_cv = soc;
		...
		int rbatt, int fcc_uah, int uuc_uah, int cc_uah)
{
	...
	if (oem_wlc_bms_is_wireless_charging()) {
		oem_wlc_print(3, "Now_Wireless_charging\n");
		if (chip->soc_at_cv == -EINVAL) {
			ibat_ua = ibat_ua - oem_wlc_icharge;
		}
		else {
			ibat_ua = (-1) * oem_wlc_icharge;
		}
		oem_wlc_charging_adjustments(chip, soc, ibat_ua, cc_uah);
		soc = charging_adjustments(chip, soc, vbat_uv, ibat_ua,
				batt_temp, chargecycles,
				fcc_uah, cc_uah, uuc_uah);
		oem_wlc_adjust_ocv_and_cc_in_cv(chip, soc, batt_temp, chargecycles,
				fcc_uah, cc_uah, uuc_uah);
		goto out;
	}
					...
					int batt_temp, int chargecycles)
{
	...
	oem_wlc_bms_calculate_wlc_ocv(calculated_soc, batt_temp, last_chargecycles);
	oem_wlc_bms_eoc_check(calculated_soc);
...
static int report_state_of_charge(struct pm8921_bms_chip *chip)
{
	...
	if (oem_wlc_bms_is_wireless_charging()) {
		soc = oem_wlc_scale_soc_while_chg(soc, last_soc);
	}
...
static struct pm8921_bms_correction pm8921_bms_correction_data[] = {
	...
	{CHG_STATE_WIRELESS       , CHG_MODE_CHARGING , CHG_CONDITION_4340MV     , CHG_CONDITION_4340MV     },
};
...
#define SOC_CHARGE_4240MV      (96  << SOC_SHIFT)
#define SOC_CHARGE_4340MV_INTE (100  << SOC_SHIFT)
#define SOC_CHARGE_4240MV_INTE (96  << SOC_SHIFT)
...
static int oem_pm8921_bms_correct_soc(int chg_state, int in_soc, int uim_valid)
{
	...
	if(chg_state == CHG_STATE_WIRELESS) {
		return result_soc_ui;
	}
...
#endif
{
	...
	if(oem_wlc_bms_is_wireless_charging()) {
		soc_ui = oem_wlc_bms_correct_soc(chg_state, soc);
	}
	else {
...
void pm8921_bms_charging_began(void)
{
	...
	if(oem_wlc_bms_is_wireless_charging()) {
		...
		pm8921_bms_stop_ocv_updates();
		oem_wlc_icharge = OEM_WLC_ICHARGE_DEFAULT;
		oem_wlc_icharge_at_cv = oem_wlc_icharge;
		oem_wlc_bms_estimate_wlc_ocv(&ocv_uv, OEM_WLC_ICHARGE_DEFAULT);
		oem_wlc_print(3, "start_percent = %u%% ocv_uv = %d\n", the_chip->start_percent, ocv_uv);
		for(i=0; i < OEM_WLC_OCV_MA_CNT; i++) {
			oem_wlc_ocv_est_work[i] = ocv_uv;
		}
		oem_wlc_ocv_ma_index = 0;
		oem_wlc_eoc_flag = 0;
	}
}
...
static int oem_wlc_bms_is_wireless_charging(void)
{
	int val = pm8921_is_wireless_charging();
...
static int oem_wlc_bms_estimate_wlc_ocv(int *wlc_ocv, int wlc_ichg)
{
	...
	*wlc_ocv = vbat_uv + ( (ibat_ua - wlc_ichg) * rbatt_mohm) / 1000;
	oem_wlc_print(3, "vbat_uv = %d wlc_ocv = %d \n",
	        vbat_uv, *wlc_ocv);
	return rc;
}
...
static void oem_wlc_bms_calculate_wlc_ocv(int in_soc, int batt_temp, int chargecycles)
{
	...
	if (!oem_wlc_bms_is_wireless_charging()) {
		oem_wlc_print(5, "Not Wireless Charging\n");
		return;
	}

	for(i=0, ok_cnt=0; i < OEM_WLC_OCV_CNT_MAX &&
	         ok_cnt < OEM_WLC_OCV_AVG_CNT; i++) {
		rc = oem_wlc_bms_estimate_wlc_ocv(&ocv_est_tmp, OEM_WLC_ICHARGE_DEFAULT);
		if (rc < 0) {
			oem_wlc_print(3, "estimate wlc ocv err = %d\n", rc);
			continue;
		}
		
		ocv_est_total += ocv_est_tmp;
		ok_cnt++;
	}
	
	ocv_est_avg = ocv_est_total / ok_cnt;
	oem_wlc_print(3, "i = %d, ok_cnt = %d, ocv_est_total = %d, ocv_est_avg = %d \n",
	        i, ok_cnt, ocv_est_total, ocv_est_avg);
	
	oem_wlc_ocv_ma_index = (oem_wlc_ocv_ma_index + 1) % OEM_WLC_OCV_MA_CNT;
	oem_wlc_ocv_est_work[oem_wlc_ocv_ma_index] = ocv_est_avg;
	for(i=0; i < OEM_WLC_OCV_MA_CNT; i++) {
		ocv_est_ma_total += oem_wlc_ocv_est_work[i];
	}
	ocv_est_ma = ocv_est_ma_total / OEM_WLC_OCV_MA_CNT;
	oem_wlc_print(3, "ocv_est_ma = %d, ocv_est_ma_total = %d, oem_wlc_ocv_ma_index = %d,  ma = %d\n",
	        ocv_est_ma, ocv_est_ma_total, oem_wlc_ocv_ma_index, OEM_WLC_OCV_MA_CNT);
	
	oem_wlc_print(3, "ocv_est_avg = %d ocv_est_ma = %d\n",
	        ocv_est_avg, ocv_est_ma);
	
    pc_est = calculate_pc(the_chip, ocv_est_ma, batt_temp, last_chargecycles);
        
    fcc_uah = calculate_fcc_uah(the_chip, batt_temp, last_chargecycles);
    uuc_uah = (fcc_uah * 3) / 100;
    soc_est = div_s64((s64)fcc_uah * pc_est - uuc_uah*100,
						(s64)fcc_uah - uuc_uah);
	soc_est = bound_soc(soc_est);

	oem_wlc_print(3, "ocv_est = %03d ocv = %d soc_est = %03d\n", pc_est, ocv_est_ma, soc_est);
	
	oem_wlc_bms_icharge_adjust(in_soc, soc_est);
}

int oem_pm8921_bms_get_wireless_eoc(void)
{
	oem_wlc_print(5, "oem_wlc_eoc_flag = %d\n", oem_wlc_eoc_flag);
	return oem_wlc_eoc_flag;
}
EXPORT_SYMBOL(oem_pm8921_bms_get_wireless_eoc);
...
static void oem_wlc_bms_eoc_check(int soc)
{
	...
	if (soc >= OEM_WLC_EOC_SOC && vbat_uv >= OEM_WLC_EOC_VBATT_UV) {
	     count++;
	}
	...
	oem_wlc_print(3, "soc = %d vbat_uv = %d oem_wlc_eoc_flag = %d \n", soc, vbat_uv, oem_wlc_eoc_flag);
}

static void oem_wlc_bms_icharge_adjust(int in_soc, int soc_est)
{
	if (the_chip->soc_at_cv == -EINVAL) {
		if(in_soc >= OEM_WLC_SOC_THRESHOLD) {
			if(abs(in_soc - soc_est) > OEM_WLC_SOC_LOW_DELTA) {
				if((in_soc - soc_est) > OEM_WLC_SOC_MID_DELTA ) {
					oem_wlc_icharge = OEM_WLC_ICHARGE_VERY_LOW;
				}
				
				else if((in_soc - soc_est) > OEM_WLC_SOC_LOW_DELTA ) {
					oem_wlc_icharge = OEM_WLC_ICHARGE_LOW;
				}
				
				else if((soc_est - in_soc) > OEM_WLC_SOC_MID_DELTA){
					oem_wlc_icharge = OEM_WLC_ICHARGE_VERY_HIGH;
				}
				else if((soc_est - in_soc) > OEM_WLC_SOC_LOW_DELTA){
					oem_wlc_icharge = OEM_WLC_ICHARGE_HIGH;
				}
			}
			
			else if(abs(in_soc - soc_est) <= OEM_WLC_SOC_ACCEPTABLE) {
				oem_wlc_icharge = OEM_WLC_ICHARGE_DEFAULT;
			}
		}
		else {
			if(abs(in_soc - soc_est) > OEM_WLC_SOC_MID_DELTA) {
				if((in_soc - soc_est) > OEM_WLC_SOC_HIGH_DELTA ) {
					oem_wlc_icharge = OEM_WLC_ICHARGE_VERY_LOW;
				}
				
				else if((in_soc - soc_est) > OEM_WLC_SOC_MID_DELTA ) {
					oem_wlc_icharge = OEM_WLC_ICHARGE_LOW;
				}
				
				else if((soc_est - in_soc) > OEM_WLC_SOC_HIGH_DELTA){
					oem_wlc_icharge = OEM_WLC_ICHARGE_VERY_HIGH;
				}
				else if((soc_est - in_soc) > OEM_WLC_SOC_MID_DELTA){
					oem_wlc_icharge = OEM_WLC_ICHARGE_HIGH;
		...
		oem_wlc_print(3, "in_soc = %d soc_est = %d oem_wlc_icharge = %d \n", 
	        	in_soc, soc_est, oem_wlc_icharge);
	}
}
...
static void oem_wlc_bms_recalculate_cc_uah(int *cc_uah, int delta_time_s)
{
	...
	if (!oem_wlc_bms_is_wireless_charging()) {
		*cc_uah -= oem_wlc_cc_uah_total;
		oem_wlc_print(3, "Not Wireless charging oem_wlc_cc_uah_total = %d\n", oem_wlc_cc_uah_total);
		return;
	}
	
	delta_cc_uah = div_s64(delta_time_s * oem_wlc_icharge, 3600);
	oem_wlc_cc_uah_total += delta_cc_uah;
	*cc_uah -= oem_wlc_cc_uah_total;
	oem_wlc_print(3, "oem_wlc_cc_uah_total = %duAh delta_time_s = %dsec oem_wlc_icharge = %duA\n",
	        oem_wlc_cc_uah_total, delta_time_s, oem_wlc_icharge);
}

static void oem_wlc_charging_adjustments(struct pm8921_bms_chip *chip, int soc, int ibat_ua, int cc_uah)
{
	static int prev_cc_uah = 0;

	if (chip->soc_at_cv == -EINVAL) {
		/* In constant current charging return the calc soc */
		if (soc < OEM_WLC_CV_SOC) {
			oem_wlc_print(3, "CC CHG SOC %d\n", soc);
			return;
		}

		if(ibat_ua > 0){
			oem_wlc_print(3, "CC CHG ibat_ua %d\n", ibat_ua);
			return;
		}
		...
		if (soc >= OEM_WLC_CV_SOC) {
			chip->soc_at_cv = soc;
			chip->prev_chg_soc = soc;
			chip->ibat_at_cv_ua = (ibat_ua + OEM_WLC_ICHARGE_100) * (100 - soc) / (100 - OEM_WLC_CV_SOC) - OEM_WLC_ICHARGE_100;
			oem_wlc_icharge_at_cv = (-1) * chip->ibat_at_cv_ua;
			oem_wlc_icharge = oem_wlc_icharge_at_cv;
			prev_cc_uah = cc_uah;
			oem_wlc_print(3, "CC_TO_CV ibat_ua = %d CHG SOC %d oem_wlc_icharge_at_cv %d\n",
					ibat_ua, soc, oem_wlc_icharge_at_cv);
		}
		return;
	}

	oem_wlc_icharge = oem_wlc_icharge * OEM_WLC_ICHG_DECREASE_RATE / OEM_WLC_ICHG_DECREASE_SCALE;
	oem_wlc_print(3, "Decrease oem_wlc_icharge = %d soc = %d\n", oem_wlc_icharge, soc);
				...
				int fcc_uah, int cc_uah, int uuc_uah)
{
	...
	if (chip->soc_at_cv != -EINVAL) {
		oem_wlc_print(3, "soc = %d batt_temp = %d last_ocv_uv = %d max_voltage_uv = %d\n", soc, batt_temp, chip->last_ocv_uv, chip->max_voltage_uv);
		chip->last_ocv_uv = chip->max_voltage_uv;
		reset_cc(chip);
		chip->last_cc_uah = 0;
		oem_wlc_cc_uah_total = 0;
...
static int oem_wlc_scale_soc_while_chg(int new_soc, int prev_soc)
{
	...
	if(chg_time_sec <= OEM_WLC_BMS_START_TIME)
		return prev_soc;
...
static int oem_wlc_bms_correct_soc(int chg_state, int in_soc)
{
	...
	if (low_batt_detection != LOW_BATT_DETECTION_ON) {
		if(in_soc >= (SOC_PERCENT_FULL >> SOC_SHIFT)) {
			result_soc_ui = (SOC_PERCENT_FULL_NEAR + SOC_ROUND_THRESHOLD) >> SOC_SHIFT;
		} else {
			result_soc_ui = in_soc;
		}
		current_soc_ui = current_soc;
		last_time_soc_ui = current_soc;
	}
	
	if(current_soc >= SOC_PERCENT_FULL_NEAR && oem_pm8921_bms_get_wireless_eoc()) {
		result_soc_ui = (SOC_PERCENT_FULL_NEAR + SOC_ROUND_THRESHOLD) >> SOC_SHIFT;
		current_soc = SOC_PERCENT_FULL;
		last_time_soc = SOC_PERCENT_FULL;
		current_soc_ui = SOC_PERCENT_FULL;
		last_time_soc_ui = SOC_PERCENT_FULL;
	}

	last_time_correction_mode = CHG_MODE_CHARGING;
	oem_wlc_print(3, "result_soc_ui = %d current_soc_ui = %d current_soc = %d in_soc = %d\n",
                      result_soc_ui, current_soc_ui, current_soc, in_soc);
	return result_soc_ui;
}

static int oem_wlc_set_shutdown_time(const char *val, struct kernel_param *kp)
{
	unsigned long now_tm_sec = 0;

	param_set_int(val, kp);
	get_current_time(&now_tm_sec);
	oem_wlc_shutdown_time = now_tm_sec;
	oem_wlc_print(3, "oem_wlc_shutdown_time = %lu \n", oem_wlc_shutdown_time);
	kdnand_id_write(DNAND_ID_KERNEL_18, 144, (uint8_t*)&now_tm_sec, sizeof(unsigned long));
	return 0;
}
module_param_call(oem_wlc_set_shutdown_time_flag, oem_wlc_set_shutdown_time,
					param_get_int, &oem_wlc_set_shutdown_time_flag, 0644);

static int oem_wlc_check_shutdown_time(void)
{
	unsigned long now_tm_sec = 0;
	unsigned long diff_sec = 0;
	int ret = 0;

	oem_wlc_print(3, "wlc_flag = %d \n", oem_param_bms.wlc_flag);

	if(oem_param_bms.wlc_flag==1) {
		get_current_time(&now_tm_sec);
		oem_wlc_boot_time = now_tm_sec;
		oem_wlc_shutdown_time = oem_param_bms.shutdown_time;

		if(oem_wlc_boot_time > oem_wlc_shutdown_time) {
			diff_sec = oem_wlc_boot_time - oem_wlc_shutdown_time;
		}
		oem_wlc_print(3, "boot time = %lu shutdown time = %lu diff_time = %lu \n",
		        oem_wlc_boot_time, oem_wlc_shutdown_time, diff_sec);

		if(diff_sec > OEM_WLC_SHUTDOWN_TIME) {
			ret = 1;
		}
	}
	return ret;
...
static int __devinit pm8921_bms_probe(struct platform_device *pdev)
{
	...
	chip->ignore_shutdown_soc = oem_wlc_check_shutdown_time();
