...
#define CXD2235_POWER_GPIO_NO			9
...
#define RFS_POLL_GPIO_NO				RFS_MSM_GPIO_NUM
...
static ssize_t hsel_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	if ( 1 > len ) {
		KNFC_LOG_E("length check len = %d", len);
		return -EIO;
	}
	ret = gpio_get_value( HSEL_GPIO_NUM );
	if ( 0 > ret ) {
	...
	KNFC_LOG_D("END hsel_val = %d, len = %d", hsel_val, len);
...
static ssize_t hsel_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
	...
	gpio_set_value(HSEL_GPIO_NUM , hsel_val );
...
static ssize_t rws_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	KNFC_LOG_D("END rws_val = %d, len = %d", rws_val, len);
...
static ssize_t rate_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	KNFC_LOG_D("END rate_val = %d, len = %d", rate_val, len);
...
static ssize_t rfs_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	ret = gpio_get_value( RFS_MSM_GPIO_NUM );
	if (ret < 0) {
		KNFC_LOG_E("gpio_get_value ret = %d", ret);
		return ret;
	}
	...
	KNFC_LOG_D("END on = %d, len = %d", on, len);
...
static int pon_release(struct inode *inode, struct file *file)
{
	...
	gpio_set_value(MFD_PON_GPIO_NUM , PON_DEV_LOW);
...
static void int_poll_work_func(struct work_struct *work)
{
	...
	read_value = gpio_get_value(INT_POLL_GPIO_NO);
	...
	if (read_value < 0) {
		...
		if (irq_set_irq_type(gpio_to_irq(INT_POLL_GPIO_NO), irqflag)){
			KNFC_LOG_E("set_irq_type irqflag = %ld", irqflag);
		}
	}
	
	enable_irq(gpio_to_irq(INT_POLL_GPIO_NO));
...
static irqreturn_t int_poll_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(gpio_to_irq(INT_POLL_GPIO_NO));
...
static ssize_t int_poll_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	KNFC_LOG_D("END len = %d, cret = %d", len, cret);
...
static int int_poll_open(struct inode *inode, struct file *file)
{
	...
	ret = gpio_get_value(INT_POLL_GPIO_NO);
	if (ret < 0) {
		int_d->open_flag = 0;
		KNFC_LOG_E("gpio_get_value ret = %d", ret);
		return -EIO;
	...
	if (request_irq(gpio_to_irq(INT_POLL_GPIO_NO), int_poll_irq_handler, irqflag, INT_POLL_DEV_NAME, (void*)int_d)) {
		int_d->open_flag = 0;
		KNFC_LOG_E("request_irq irqflag = %ld", irqflag);
		return -EIO;
	}
	
	if (enable_irq_wake(gpio_to_irq(INT_POLL_GPIO_NO))){
		
		KNFC_LOG_E("enable_irq_wake");
		
		free_irq(gpio_to_irq(INT_POLL_GPIO_NO), (void *)int_d);
...
static int int_poll_release(struct inode *inode, struct file *file)
{
	...
	if (disable_irq_wake(gpio_to_irq(INT_POLL_GPIO_NO))){
...
static void rfs_poll_work_func(struct work_struct *work)
{
	...
	read_value = gpio_get_value(RFS_POLL_GPIO_NO);
	...
	if (read_value < 0) {
		...
		if (irq_set_irq_type(gpio_to_irq(RFS_POLL_GPIO_NO), irqflag)){
			KNFC_LOG_E("set_irq_type irqflag = %ld", irqflag);
		}
	}
	
	enable_irq(gpio_to_irq(RFS_POLL_GPIO_NO));
	...
	KNFC_LOG_D("END read_value = %d, old_value = %d, rfs_d->read_error = %d", read_value, old_value, rfs_d->read_error);
}
...
static irqreturn_t rfs_poll_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(gpio_to_irq(RFS_POLL_GPIO_NO));
...
static int rfs_poll_open(struct inode *inode, struct file *file)
{
	...
	ret = gpio_get_value(RFS_POLL_GPIO_NO);
	if (ret < 0) {
		rfs_d->open_flag = 0;
		KNFC_LOG_E("gpio_get_value ret = %d", ret);
		return -EIO;
	...
	if (request_irq(gpio_to_irq(RFS_POLL_GPIO_NO), rfs_poll_irq_handler, irqflag, RFS_POLL_DEV_NAME, (void*)rfs_d)) {
		rfs_d->open_flag = 0;
		KNFC_LOG_E("request_irq irqflag = %ld", irqflag);
		return -EIO;
	}
	
	if (enable_irq_wake(gpio_to_irq(RFS_POLL_GPIO_NO))){
		
		KNFC_LOG_E("enable_irq_wake");
		
		free_irq(gpio_to_irq(RFS_POLL_GPIO_NO), (void *)rfs_d);
...
static int rfs_poll_release(struct inode *inode, struct file *file)
{
	...
	if (disable_irq_wake(gpio_to_irq(RFS_POLL_GPIO_NO))){
		KNFC_LOG_E("disable_irq_wake");
	}
	free_irq(gpio_to_irq(RFS_POLL_GPIO_NO), (void *)rfs_d);
...
static void intu_poll_work_func(struct work_struct *work)
{
	...
	read_value = gpio_get_value(INTU_POLL_GPIO_NO);
	...
	if (read_value < 0) {
		...
		if (irq_set_irq_type(gpio_to_irq(INTU_POLL_GPIO_NO), irqflag)){
			KNFC_LOG_E("set_irq_type irqflag = %ld", irqflag);
		}
	}
	
	enable_irq(gpio_to_irq(INTU_POLL_GPIO_NO));
...
static irqreturn_t intu_poll_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(gpio_to_irq(INTU_POLL_GPIO_NO));
...
static int intu_poll_open(struct inode *inode, struct file *file)
{
	...
	ret = gpio_get_value(INTU_POLL_GPIO_NO);
	if (ret < 0) {
		intu_d->open_flag = 0;
		KNFC_LOG_E("gpio_get_value ret = %d", ret);
		return -EIO;
	...
	if (request_irq(gpio_to_irq(INTU_POLL_GPIO_NO), intu_poll_irq_handler, irqflag, INTU_POLL_DEV_NAME, (void*)intu_d)) {
		intu_d->open_flag = 0;
		KNFC_LOG_E("request_irq irqflag = %ld", irqflag);
		return -EIO;
	}
	
	if (enable_irq_wake(gpio_to_irq(INTU_POLL_GPIO_NO))){
		
		KNFC_LOG_E("enable_irq_wake");
		
		free_irq(gpio_to_irq(INTU_POLL_GPIO_NO), (void *)intu_d);
...
static int intu_poll_release(struct inode *inode, struct file *file)
{
	...
	if (disable_irq_wake(gpio_to_irq(INTU_POLL_GPIO_NO))){
		KNFC_LOG_E("disable_irq_wake");
	}
	free_irq(gpio_to_irq(INTU_POLL_GPIO_NO), (void *)intu_d);
...
static ssize_t available_poll_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	KNFC_LOG_D("available poll read END read size = %d ", len);
...
static ssize_t felicadev_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	KNFC_LOG_D("felicadev read START len = %d",len);
	...
	if( len != slen ){
		if( len < slen ){
			KNFC_LOG_E("size unmatch len = %d slen = %d",len,slen);
			up( &chardev_sem );
			return -EFAULT;
		}
	}
	...
	g_functype = -1;
	up( &chardev_sem );
	KNFC_LOG_D("felicadev read END param size = %d ret size = %d ", len, slen);
...
static ssize_t felicadev_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
	...
	g_functype = -1;
	up( &chardev_sem );
	KNFC_LOG_D("felicadev write END retsize = %d paramsize = %d",nRet,len);
	return nRet;
}
static long felicadev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static ssize_t cxd2235_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	if( 0x00 == g_cxd_buf[1] ){
		...
		if( len != ssize ){
			KNFC_LOG_E("size unmatch len = %d ssize = %d",len,ssize);
			if( len < ssize ){
				up( &chardev_sem );
				return -EFAULT;
			}
...
static ssize_t cxd2235power_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
	...
	gpio_set_value(CXD2235_POWER_GPIO_NO , on );
...
static ssize_t knfcvbus_read(struct file *file, char __user * buf, size_t len, loff_t * ppos)
{
	...
	KNFC_LOG_D("END rate_val = %d, len = %d", (int)vbus_val, len);
